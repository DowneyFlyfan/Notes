...
- 在数学中，一个映射$f:M \xrightarrow{} N$被称为微分同胚，如果它满足以下三个条件：

- 直观理解: 一个物体不被撕裂或拼接所获得的结果

| 特性 (Property) | 描述 (Description) |
|---|---|
| 光滑性 (Smoothness) | 映射 $f$ 是光滑的。这意味着 $f$ 在所有阶导数上都是连续的。在欧几里得空间中，这通常指 $f$ 的各分量函数都是无限可微的（C<sup>∞</sup>）。 |
| 双射性 (Bijectivity) | 映射 $f$ 是双射的，即它是单射且满射. 这意味着对于 N 中的每一个点，在 M 中都有且仅有一个点映射到它。 |
| 逆映射的光滑性 (Smoothness o$f$ the Inverse) | 映射 $f$ 的逆映射 $f$<sup>-1</sup>:N→M 也是光滑的。 |
